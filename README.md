...
1. Соединение идёт только напрямую и локально.
2. При попытке отмены (Ctrl+C):
   1. Если происходит DTLS Handshake, то он не отменяется и выводит ошибку о том, что сервер не отвечает.
   2. Если соединение установлено, то он начинает плодить соединения
...
Этот проект принимает отсылает DTLS пакеты скозь TURN сервера, используя их как прокси сервера меж условным клиентом и сервером, данный проект является именно первым.
